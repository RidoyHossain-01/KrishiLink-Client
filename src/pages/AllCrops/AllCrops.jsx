...
const AllCrops = () => {
  ...
  const handleSearch = (e) => {
    ...
    axios(
      `https://krishi-link-server-omega.vercel.app/search?search=${search}`
    ).then((res) => {
      setCrops(res.data);
    });
    // console.log(search);
  ...
  return (
    <div data-aos="fade-up">
      <div className="text-center my-6">
        <h1 className="text-5xl text-green-700">Explore All Available Crops</h1>
        <p className="text-xl mt-3 mb-8 text-orange-500">
          Find fresh, authentic, and locally grown crops posted by trusted
          farmers and sellers.
        </p>
      </div>
      <div className="flex flex-col-reverse md:flex-row  items-center justify-between my-4">
        <p className=" text-2xl">
          <span className="font-semibold">{crops.length}</span> Crops available
        </p>
        <form
          onSubmit={handleSearch}
          className="text-right my-4 flex items-center justify-end"
        >
          <div className="flex items-center justify-center p-5">
            <div className="rounded-lg bg-green-100 p-5">
              <div className="flex">
                ...
                <button
                  type="submit"
                  className="bg-green-500 px-4 py-2 rounded-tr-lg rounded-br-lg text-white font-semibold hover:bg-green-700 transition-colors"
                >
                  Search
                </button>
              </div>
      ...
      <Link
        className="btn bg-purple-500 text-white text-lg flex justify-center"
        onClick={() => {
          navigate(0);
        }}
      >
